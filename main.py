...
class Bubble(pygame.sprite.Sprite):
    ...
    def find_position(self, g):
        x1 = random.randint(self.r + Settings.border, Settings.window_width - self.r - Settings.border)
        y1 = random.randint(self.r + Settings.border, Settings.window_height - self.r - Settings.border)
        r1 = self.r
        if len(g.bubbles) <= 0:
    ...
    def update(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = c
        collision_bubbles = pygame.sprite.spritecollide(self, game.bubbles, False, pygame.sprite.collide_mask)
        if len(collision_bubbles) > 0:
            for bubble in collision_bubbles:
                if bubble != self:
                    collision_sound = pygame.mixer.Sound(os.path.join(Settings.path_sound, "Collision.mp3"))
...
class Game(object):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                for bubble in self.bubbles:
                    ...
                    if hits == True:
                        burst_sound = pygame.mixer.Sound(os.path.join(Settings.path_sound, "Burst.mp3"))
    ...
    def update(self):
        ...
        if self.t >= Settings.timeunit:
            self.t = 0
            if len(self.bubbles) <= (Settings.window_width + Settings.window_height) / 100:
                self.bubbles.add(Bubble("bubble.png", self))
                spawn_sound = pygame.mixer.Sound(os.path.join(Settings.path_sound, "spawn.mp3"))
